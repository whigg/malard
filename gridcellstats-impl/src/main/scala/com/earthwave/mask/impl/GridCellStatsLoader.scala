...
class GridCellStatsLoader extends LagomApplicationLoader {
  ...
  override def describeService = Some(readDescriptor[GridCellStatsService])
}
...
abstract class GridCellStatsApplication(context: LagomApplicationContext)
  ...
  override lazy val lagomServer: LagomServer = serverFor[GridCellStatsService](wire[GridCellStatsServiceImpl])
